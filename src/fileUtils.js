...
function getFiles(rootPath, currentPath, ignoreList, ignoreDotFiles, ignoreMinFiles) {
  ...
  ignoreList = ignoreList.concat(["package-lock.json", "dist", "obj", "bin", ".vscode", ".next"]);
  for (const itemName of directoryItems) {
    ...
    if (itemStat.isDirectory()) {
      ...
      if ((ignoreDotFiles && itemName.startsWith(".")) || 
          (ignoreMinFiles && (itemExtension === ".min.js" || itemExtension === ".min.css"))) {
        continue;
      }
...
module.exports = { getFiles, normalizeIgnorePath };
